import type { Flow, FlowAuth, FlowBodyProperty, FlowEndpoint, FlowParameter, FlowResponse, HttpMethod, ParamType } from "../components/FlowsManager";
import type {
    FlowsResponse,
    ...
    return data.data.flows.map(flow => {
        ...
        for (const path in flow.paths) {
            ...
            for (const method in methods) {
                ...
                // Responses
                const responses: FlowResponse[] = Object.entries(op.responses || {}).map(
                    ([statusCode, resp]: [string, BackendFlowResponse]) => ({
                        id: crypto.randomUUID(),
